...
class Solver(object):
    def __init__(self, config, train_loader, valid_loader, test_loader):
        ...
        # optional max steps per epoch for quick smoke tests
        self.max_steps = getattr(config, 'max_steps', None)
        # path for saving last-epoch model (always saved)
        self.net_path_last = os.path.join(self.model_path, self.report_name + '_last.pkl')
    ...
    def train(self):
        ...
        # scaling factor used in loss composition (kept from original implementation)
        factor = 1
        self.build_model()
        
        # Prepare results files and CSV writers
        if os.path.isfile(self.net_path):
            # if pretrained model exists, append to existing logs
        ...
        for epoch in range(self.num_epochs):
            ...
            # per-epoch progress bar and optional max steps
            total_batches = len(self.train_loader)
            if self.max_steps is not None:
                total_batches = min(total_batches, int(self.max_steps))

            step = 0
            train_iter = self.train_loader
            for i, (image, GT, name) in enumerate(tqdm(train_iter, total=total_batches, desc=f"Train E{epoch+1}/{self.num_epochs}")):

                # allow limiting number of training steps per epoch for quick tests
                step += 1
                if (self.max_steps is not None) and (step >= int(self.max_steps)):
                    # process this batch then break
                    pass
            ...
            val_total = len(self.valid_loader)
            if self.max_steps is not None:
                val_total = min(val_total, int(self.max_steps))
            step_val = 0
            for i, (image, GT, name) in enumerate(tqdm(self.valid_loader, total=val_total, desc=f"Val E{epoch+1}/{self.num_epochs}")):
                ...
                DC += get_DC(SR_f,GT_f)[1]
                length += 1
                # respect max_steps for validation as well
                step_val += 1
                if (self.max_steps is not None) and (step_val >= int(self.max_steps)):
                    break
        ...
        # save last-epoch model so short/smoke runs still produce a checkpoint
        try:
            torch.save(self.model, self.net_path_last)
            self.report.write('\nSaved last-epoch model to: %s' % self.net_path_last)
            print('Saved last-epoch model to', self.net_path_last)
        except Exception:
            pass
